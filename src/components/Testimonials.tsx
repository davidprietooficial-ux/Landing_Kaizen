'use client'

import { useEffect, useRef, useState } from 'react'
import { CLIENTS as CLIENTS_ES, TESTIMONIALS as TESTIMONIALS_ES, TESTIMONIALS_BADGE as BADGE_ES } from '@/lib/config'
import { CLIENTS as CLIENTS_EN, TESTIMONIALS as TESTIMONIALS_EN, TESTIMONIALS_BADGE as BADGE_EN } from '@/lib/config.en'
import type { Locale } from '@/lib/locale'
import { ChevronLeft, ChevronRight, Star } from './Icons'

const GAP = 24 // debe coincidir con el gap del track en CSS

const COPY = {
  es: {
    eyebrow: 'Testimonios',
    titleLead: 'Lo que dicen nuestros',
    titleEm: 'clientes',
    avgAria: (avg: number) => `Promedio: ${avg} de 5 estrellas`,
    starsAria: (n: number) => `${n} de 5 estrellas`,
    prev: 'Testimonio anterior',
    next: 'Testimonio siguiente',
    logostripAria: 'Marcas con las que hemos trabajado',
    localeCode: 'es-CO',
  },
  en: {
    eyebrow: 'Testimonials',
    titleLead: 'What our',
    titleEm: 'clients say',
    avgAria: (avg: number) => `Average: ${avg} out of 5 stars`,
    starsAria: (n: number) => `${n} out of 5 stars`,
    prev: 'Previous testimonial',
    next: 'Next testimonial',
    logostripAria: 'Brands we have worked with',
    localeCode: 'en-US',
  },
}

// Estrella con relleno parcial: la dorada se recorta al % que corresponda
// sobre una base apagada → puede mostrar 4.5, 4.7, etc.
function PartialStar({ fill }: { fill: number }) {
  return (
    <span className="pstar" aria-hidden="true">
      <Star size={20} className="pstar__bg" />
      {fill > 0 && (
        <span className="pstar__fill" style={{ width: `${fill * 100}%` }}>
          <Star size={20} />
        </span>
      )}
    </span>
  )
}

// Retrato con respaldo: si la foto aún no está subida, muestra la inicial.
function Avatar({ src, name }: { src: string; name: string }) {
  const [failed, setFailed] = useState(false)
  if (failed) return <span className="t-card__ph" aria-hidden="true">{name.charAt(0)}</span>
  // eslint-disable-next-line @next/next/no-img-element
  return <img src={src} alt="" loading="lazy" width={48} height={48} onError={() => setFailed(true)} />
}

export default function Testimonials({ locale = 'es' }: { locale?: Locale }) {
  const t = COPY[locale]
  const TESTIMONIALS = locale === 'en' ? TESTIMONIALS_EN : TESTIMONIALS_ES
  const CLIENTS = locale === 'en' ? CLIENTS_EN : CLIENTS_ES
  const TESTIMONIALS_BADGE = locale === 'en' ? BADGE_EN : BADGE_ES

  const N = TESTIMONIALS.length
  // Triplicado: el carrusel vive en la copia del medio y salta sin animación al
  // pasarse de los bordes → efecto de scroll infinito en ambas direcciones.
  const LOOP = [...TESTIMONIALS, ...TESTIMONIALS, ...TESTIMONIALS]
  // Promedio real de todas las calificaciones, a 1 decimal (p. ej. 4.5).
  const AVG = Math.round((TESTIMONIALS.reduce((s, x) => s + x.stars, 0) / N) * 10) / 10

  const [idx, setIdx] = useState(N)
  const [anim, setAnim] = useState(false)
  const [paused, setPaused] = useState(false)
  const trackRef = useRef<HTMLDivElement>(null)
  const [step, setStep] = useState(0)

  // Mide ancho de card + gap (cambia con el viewport).
  useEffect(() => {
    const measure = () => {
      const first = trackRef.current?.children[0] as HTMLElement | undefined
      if (first) setStep(first.offsetWidth + GAP)
    }
    measure()
    window.addEventListener('resize', measure)
    return () => window.removeEventListener('resize', measure)
  }, [])

  // Habilita la transición después del primer posicionamiento (sin animar la carga).
  useEffect(() => {
    if (step === 0 || anim) return
    const raf = requestAnimationFrame(() => requestAnimationFrame(() => setAnim(true)))
    return () => cancelAnimationFrame(raf)
  }, [step, anim])

  // Autoplay cada 3s; se pausa con el mouse encima.
  useEffect(() => {
    if (paused) return
    const timer = window.setInterval(() => setIdx((i) => i + 1), 3000)
    return () => window.clearInterval(timer)
  }, [paused])

  // Fuera de la copia del medio → espera a que termine la transición (0.8s) y salta.
  useEffect(() => {
    if (idx >= N && idx < 2 * N) return
    const timer = window.setTimeout(() => {
      setAnim(false)
      setIdx((i) => (i >= 2 * N ? i - N : i + N))
    }, 820)
    return () => window.clearTimeout(timer)
  }, [idx, N])

  return (
    <section
      id="testimonios"
      className="section tstm"
      onMouseEnter={() => setPaused(true)}
      onMouseLeave={() => setPaused(false)}
    >
      <div className="container">
        <div className="tstm__head">
          <div>
            <span className="eyebrow">{t.eyebrow}</span>
            <h2 className="tstm__title">
              {t.titleLead} <em>{t.titleEm}</em>
            </h2>
          </div>
          <div className="tstm__badge">
            <span className="tstm__stars" role="img" aria-label={t.avgAria(AVG)}>
              {Array.from({ length: 5 }, (_, i) => (
                <PartialStar key={i} fill={Math.max(0, Math.min(1, AVG - i))} />
              ))}
            </span>
            <span className="mono">{TESTIMONIALS_BADGE.label} {AVG.toLocaleString(t.localeCode)}/5</span>
          </div>
        </div>
      </div>

      <div className="tstm__viewport">
        <div
          ref={trackRef}
          className="tstm__track"
          style={{
            transform: `translateX(${-idx * step}px)`,
            transition: anim ? 'transform .8s cubic-bezier(.4, 0, .2, 1)' : 'none',
          }}
        >
          {LOOP.map((item, i) => (
            <figure className={`t-card${i < idx ? ' is-out' : ''}`} key={`${item.name}-${i}`} aria-hidden={i < N || i >= 2 * N || undefined}>
              <div className="t-card__top">
                <svg className="t-card__quote" width="30" height="24" viewBox="0 0 30 24" aria-hidden="true">
                  <path
                    fill="currentColor"
                    d="M0 24V12.4C0 5.4 4.4 1 11.2 0l1.2 3C7.9 4.5 5.7 7 5.5 10H12v14H0Zm18 0V12.4C18 5.4 22.4 1 29.2 0l1.2 3c-4.5 1.5-6.7 4-6.9 7H30v14H18Z"
                  />
                </svg>
                <span className="t-card__stars" role="img" aria-label={t.starsAria(item.stars)}>
                  {Array.from({ length: 5 }, (_, s) => (
                    <Star key={s} size={15} className={s < item.stars ? undefined : 'dim'} />
                  ))}
                </span>
              </div>
              <blockquote>{item.quote}</blockquote>
              <figcaption className="t-card__author">
                <Avatar src={item.photo} name={item.name} />
                <div>
                  <strong>{item.name}</strong>
                  {(item.role || item.company) && <span>{[item.role, item.company].filter(Boolean).join(', ')}</span>}
                </div>
              </figcaption>
            </figure>
          ))}
        </div>
      </div>

      <div className="container tstm__nav">
        <button type="button" onClick={() => setIdx((i) => i - 1)} aria-label={t.prev}>
          <ChevronLeft size={20} />
        </button>
        <button type="button" onClick={() => setIdx((i) => i + 1)} aria-label={t.next}>
          <ChevronRight size={20} />
        </button>
      </div>

      {/* Cinta de marcas: scroll infinito automático con los chips blancos originales
          (mismas proporciones que antes: `w` = ancho del chip, `h` = alto máx del logo) */}
      {CLIENTS.length > 0 && (
        <div className="logostrip" aria-label={t.logostripAria}>
          <div className="logostrip__track">
            {[...CLIENTS, ...CLIENTS].map((c, i) => (
              <div
                className={`client-logo${c.w ? ' client-logo--wide' : ''}`}
                key={`${c.name}-${i}`}
                title={c.name}
                style={c.w ? { width: c.w } : undefined}
                aria-hidden={i >= CLIENTS.length || undefined}
              >
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={c.src} alt={c.name} loading="lazy" width={c.iw} height={c.ih} style={c.h ? { maxHeight: c.h } : undefined} />
              </div>
            ))}
          </div>
        </div>
      )}
    </section>
  )
}
